...
<div class="content">
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class=" col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2 justify-between">
            ...
            @can('media_create')
            <a href="{{route('media.create')}}" class="btn btn-primary shadow-md mr-2">Thêm mới</a>
            @endcan
        </div>
        <!-- BEGIN: Data List -->
            ...
            <table class="table table-report -mt-2">
                ...
                <tbody id="table_data" role="alert" aria-live="polite" aria-relevant="all">
                    ...
                    <tr class="odd " id="post-<?php echo $v->id; ?>">
                        ...
                        <td>
                            <a href="{{route('routerURL',['slug' => $v->slug])}}" target="_blank" class="text-primary font-bold"><?php echo $v->title; ?></a><br>
                            @foreach($v->getCategoryMediaRelationships as $kc=>$c)
                            <a class="text-danger" href="{{route('media.index',['catalogueid' => $c->id])}}"><?php echo !empty($kc == 0) ? '' : ',' ?>{{$c->title}}</a>
                            @endforeach
                        </td>
                        ...
                        <td class="table-report__action w-56 ">
                            <div class="flex justify-center items-center">
                                @can('media_edit')
                                <a class="flex items-center mr-3" href="{{ route('media.edit',['id'=>$v->id]) }}">
