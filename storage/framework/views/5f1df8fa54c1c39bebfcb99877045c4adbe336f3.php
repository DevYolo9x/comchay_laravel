        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 lg:col-span-9 space-y-6 mt-5 md:mt-0 order-0 lg:order-1">
                <div>
                    ...
                    <form class="mt-3 space-y-3" method="post" action="<?php echo e(route('chude.store')); ?>">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="flex items-center">
                            ...
                            <select name="catalogue_id" class="flex-1 form-control block w-full px-4 py-2 text-sm font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none">
                                ...
                                <option value="<?php echo e($item->id); ?>" <?php if (old('catalogue_id') == $item->id) { ?>selected<?php } ?> <?php if (request()->get('id') == $item->id) { ?>selected<?php } ?>><?php echo e($item->title); ?></option>
                                ...
                                <option value="<?php echo e($value->id); ?>" <?php if (old('catalogue_id') == $value->id) { ?>selected<?php } ?> <?php if (request()->get('id') == $value->id) { ?>selected<?php } ?>>|-----<?php echo e($value->title); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <?php endif; ?>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <?php endif; ?>
                        ...
                        <div class="flex items-center">
                            ...
                            <input type="text" name="title" class="flex-1 form-control block w-full px-4 py-2 text-sm font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" value="<?php echo e(old('title')); ?>" placeholder="" />
                        </div>
                        <div class="flex flex-col ">
                            <label class="w-[100px] font-bold mb-3">Nội dung</label>
                            <div>
                                <?php echo Form::textarea('description', old('description'), ['id' => 'ckDescription', 'class' => 'ck-editor', 'style' => 'font-size:14px;line-height:18px;border:1px solid #ddd;padding:10px']); ?>
                            </div>
                        </div>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('homepage.layout.home', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\laravel.local\resources\views/briefing/frontend/briefing/createChuDe.blade.php ENDPATH**/ ?>